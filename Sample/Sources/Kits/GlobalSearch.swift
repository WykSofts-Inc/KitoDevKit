...
enum GlobalSampleIndex {
    static let all: [SampleSearchHit] = {
        ...
        hits += kit("Permissions", symbol: "hand.raised", footnote: "Requires `import KitoPermissions`.", PermissionsSamples.sections)
        hits += kit("Biometrics", symbol: "faceid", footnote: "Requires `import KitoBiometrics`.", BiometricsSamples.sections)
        hits += kit("Keychain", symbol: "key.fill", footnote: "Requires `import KitoKeychain`.", KeychainSamples.sections)
        hits += kit("Connectivity", symbol: "wifi", footnote: "Requires `import KitoConnectivity`.", ConnectivitySamples.sections)
        hits += kit("Notifications", symbol: "bell.badge.fill", footnote: "Requires `import KitoNotifications`.", NotificationsSamples.sections)
        hits += kit("Formatting", symbol: "textformat.123", footnote: "Requires `import KitoFormatting`.", FormattingSamples.sections)
        hits += kit("Image Loader", symbol: "photo.badge.arrow.down", footnote: "Requires `import KitoImageLoader`.", ImageLoaderSamples.sections)
        hits += kit("Cart", symbol: "cart.fill", footnote: "Requires `import KitoCart`.", CartSamples.sections)
        hits += kit("Order Tracking", symbol: "shippingbox.fill", footnote: "Requires `import KitoOrderTracking`.", OrderTrackingSamples.sections)
        hits += kit("Control Center", symbol: "slider.horizontal.3", footnote: "Built with `import KitoCore` and `import KitoHaptics`.", ControlCenterSamples.sections)
        hits += kit("Settings", symbol: "gearshape.fill", footnote: "Built with `import KitoCore`.", SettingsSamples.sections)
        hits += kit("Search", symbol: "magnifyingglass", footnote: "Requires `import KitoSearch`.", SearchSamples.sections)
        hits += kit("AI Chat", symbol: "sparkles.rectangle.stack.fill", footnote: "Requires `import KitoAIChat`.", AIChatSamples.sections)
        hits += kit("Scanner", symbol: "qrcode.viewfinder", footnote: "Requires `import KitoScanner`.", ScannerSamples.sections)
        hits += kit("Tours & Tips", symbol: "hand.point.up.left.fill", footnote: "Requires `import KitoTour`.", TourSamples.sections)
        hits += kit("Product", symbol: "tag.fill", footnote: "Requires `import KitoProduct`.", ProductSamples.sections)
        hits += kit("Checkout", symbol: "creditcard.and.123", footnote: "Requires `import KitoCheckout`.", CheckoutSamples.sections)
